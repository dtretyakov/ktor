...
public class DefaultWebSocketSessionNativeImpl(
                ...
                when (frame) {
                    ...
                    is Frame.Ping -> ponger.send(frame)
                        ...
                        val frameToSend = last?.let { builder ->
                            ...
                            Frame.byType(true, frame.frameType, builder.build().readBytes())
                        } ?: frame
    ...
    private suspend fun checkMaxFrameSize(
        ...
        if (size > maxFrameSize) {
            packet?.release()
            val errorMessage = "Frame is too big: $size. Max size is $maxFrameSize"
            close(CloseReason(CloseReason.Codes.TOO_BIG, errorMessage))
            error(errorMessage)
        }
    }

    public companion object {
        private val EmptyPong = Frame.Pong(ByteReadPacket.Empty)
    }
}
